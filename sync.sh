#!/usr/bin/env bash
# 手动镜像 tianma-if/edgeever 的 main 分支。
set -euo pipefail

UPSTREAM_URL="https://github.com/tianma-if/edgeever.git"
UPSTREAM_WEB_URL="https://github.com/tianma-if/edgeever"
UPSTREAM_BRANCH="main"
MIRROR_REPO="https://github.com/hopol/EdgeEver-mir"
...
git add upstream
git commit -m "chore: mirror EdgeEver-mir source $SOURCE_VALUE ($SHORT_COMMIT)"
git tag -a "mirror-source-v$SOURCE_VALUE-$SHORT_COMMIT" -m "Mirror EdgeEver-mir source $SOURCE_VALUE ($SHORT_COMMIT)"
git push origin HEAD
git push origin "mirror-source-v$SOURCE_VALUE-$SHORT_COMMIT"
echo "已同步 EdgeEver-mir $UPSTREAM_BRANCH：$UPSTREAM_COMMIT"
